...
class UserRegisterDataTable extends DataTable {
    public function dataTable($query) {
        $query = UserRegisterLog::with('user')->select('user_register_logs.*');
        return datatables()
            ->eloquent($query)
            ->filter(function ($query) {
                if (request()->has('filter_created_at') AND request('filter_created_at') <> null) {
                    $query->where('user_register_logs.created_at', 'like', "%".escape_input(request('filter_created_at'))."%");
                }
                ...
                if (request()->has('search') AND request('search') <> null) {
                    $query->where(function($query) {
                        ...
                        ->orWhere('user_register_logs.id', 'like', "%".escape_input(request('search'))."%")
                        ->orWhere('user_register_logs.ip_address', 'like', "%".escape_input(request('search'))."%");
                    });
                }
            })
    ...
    public function query(UserRegisterLog $model) {
    ...
    protected function getColumns() {
        return [
            ...
            ['data' => 'created_at', 'name' => 'user_register_logs.created_at', 'title' => 'DIBUAT', 'width' => '180'],
            ['data' => 'user_id', 'name' => 'user.username', 'title' => 'PENGGUNA', 'width' => '120'],
            ['data' => 'ip_address', 'name' => 'user_register_logs.ip_address', 'title' => 'ALAMAT IP'],
        ];
    }
